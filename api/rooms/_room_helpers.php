...
function fetch_room_amenity_assignments(PDO $pdo, array $roomIds): array
{
    ...
    $stmt = $pdo->prepare(
        "SELECT ra.room_id, a.id, a.name AS amenity_name
         ...
         INNER JOIN property_amenities a ON a.id = ra.amenity_id
...
function normalize_room(array $room, array $images = [], array $amenityRecords = [], array $amenityCatalog = []): array
{
    ...
    $amenities = $amenityCatalog !== []
        ? array_map(static fn(array $item): string => (string) $item['amenity_name'], $amenityRecords)
        : $legacyAmenities;
    $amenityIds = array_map(static fn(array $item): int => (int) $item['id'], $amenityRecords);
    $selectedLookup = array_fill_keys($amenityIds, true);
    $catalog = array_map(static fn(array $item): array => [
        'id' => (int) $item['id'],
        'amenity_name' => (string) $item['amenity_name'],
        'selected' => isset($selectedLookup[(int) $item['id']]),
    ], $amenityCatalog);
...
function get_room_payload(PDO $pdo, bool $publicOnly = false): array
{
    ...
    $assignments = fetch_room_amenity_assignments($pdo, $roomIds);
    $catalog = list_amenities($pdo);
    return array_map(static fn($room): array => normalize_room(
        $room,
        $images[(int) $room['id']] ?? [],
