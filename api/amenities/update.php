...
try {
    ...
    $stmt = $pdo->prepare('UPDATE property_amenities SET name = :name, updated_at = NOW() WHERE id = :id');
    $stmt->execute([':id' => $id, ':name' => $name]);
    if ($stmt->rowCount() === 0) {
        $exists = $pdo->prepare('SELECT id FROM property_amenities WHERE id = :id');
        $exists->execute([':id' => $id]);
        if (!$exists->fetch()) json_response(false, 'Amenity not found.', 404);
    }
    json_response(true, 'Amenity updated.', 200, ['data' => list_amenities($pdo)]);
} catch (PDOException $e) {
    if ((string) $e->getCode() === '23000') json_response(false, 'An amenity with this name already exists.', 409);
