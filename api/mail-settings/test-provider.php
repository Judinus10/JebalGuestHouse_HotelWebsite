...
try {
    ...
    if ($provider === 'microsoft_graph' && !empty($config['oauth_client_id']) && !empty($config['encrypted_client_secret'])) send_via_microsoft_graph($config,$recipient,'Jebal Guest House mail connection test','<p>This message confirms that Microsoft Graph mail delivery is connected.</p>');
    else send_via_provider_smtp($config,$recipient,'Jebal Guest House mail connection test','<p>This message confirms that outgoing mail delivery is connected.</p>');
